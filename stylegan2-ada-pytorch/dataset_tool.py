...
def convert_dataset(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        # uint16 grayscale must use fromarray() without mode to preserve 16-bit depth.
        # Specifying mode='L' would silently truncate to 8-bit.
        if img.dtype == np.uint16:
            img = PIL.Image.fromarray(img)          # → I;16 (16-bit grayscale)
        else:
